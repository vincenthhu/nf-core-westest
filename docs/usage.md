# nf-core/westest: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/westest/usage](https://nf-co.re/westest/usage)
...
```console
nextflow run nf-core/westest --input samplesheet.csv --genome GRCh38 -profile docker
```
...
First, go to the [nf-core/westest releases page](https://github.com/nf-core/westest/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
